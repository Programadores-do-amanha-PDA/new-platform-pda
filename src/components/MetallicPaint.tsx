...
interface MetallicPaintProps {
    ...
    readonly patternSharpness?: number;
    readonly waveAmplitude?: number;
    readonly noiseScale?: number;
    readonly chromaticSpread?: number;
    readonly mouseAnimation?: boolean;
...
/**
 * Converts a hexadecimal color code to RGB values normalized between 0 and 1.
 * @param hex - A hexadecimal color string (e.g., "#FF5733" or "FF5733")
 * @returns A tuple of three numbers [red, green, blue] with values between 0 and 1.
 *          Returns [1, 1, 1] (white) if the input is invalid.
 * @example
...
/**
 * A WebGL2-based component that renders an animated metallic paint effect on a canvas.
 *
 * The component applies advanced visual effects including refraction, chromatic aberration,
 * wave animations, and mouse-based interactivity to create a dynamic metallic surface appearance.
 *
 * @component
 * @param {Object} props - The component props
 * @param {string} props.imageSrc - URL of the image to apply the metallic effect to
 * @param {number} [props.seed=42] - Random seed for consistent effect generation
 * @param {number} [props.scale=4] - Scale factor for the effect intensity
 * @param {number} [props.refraction=0.01] - Refraction strength (0-1 range)
 * @param {number} [props.blur=0.015] - Blur amount applied to the effect
 * @param {number} [props.liquid=0.75] - Liquid/fluidity factor (0-1 range)
 * @param {number} [props.speed=0.3] - Animation speed multiplier
 * @param {number} [props.brightness=2] - Brightness adjustment factor
 * @param {number} [props.contrast=0.5] - Contrast adjustment factor
 * @param {number} [props.angle=0] - Rotation angle in radians
 * @param {number} [props.fresnel=1] - Fresnel effect intensity
 * @param {string} [props.lightColor="#ffffff"] - Hex color for light reflections
 * @param {string} [props.darkColor="#000000"] - Hex color for dark areas
 * @param {number} [props.patternSharpness=1] - Sharpness of surface patterns
 * @param {number} [props.waveAmplitude=1] - Amplitude of wave distortions
 * @param {number} [props.noiseScale=0.5] - Scale of noise pattern
 * @param {number} [props.chromaticSpread=2] - Chromatic aberration spread amount
 * @param {boolean} [props.mouseAnimation=false] - Enable animation driven by mouse position
 * @param {number} [props.distortion=1] - Distortion effect intensity
 * @param {number} [props.contour=0.2] - Contour/edge definition strength
 * @param {string} [props.tintColor="#feb3ff"] - Hex color tint overlay
 *
 * @returns {JSX.Element} A canvas element with the metallic paint effect
 *
 * @example
 * <MetallicPaint
