import { SettingJustification, ClassTypesLimit } from "@/features/classrooms/settings";
import { CalculateUserAttendanceResult, CalculateUserAttendanceParams } from "../types";
...
export function calculateUserAttendance({
...
}: CalculateUserAttendanceParams): CalculateUserAttendanceResult {
    const userJustification = meeting?.justifications?.find((justification) => justification.user_email === userEmail);
    ...
    if (userJustification) {
        ...
        const bestJustification = findBestJustification(userJustification.message, availableJustifications);
    ...
    const userParticipations = meeting?.participants?.filter((participant) => participant.user_email === userEmail) || [];

    const totalMinutesAttended = Math.round(
        userParticipations.reduce((accumulator, participation) => accumulator + participation.duration, 0) / 60,
    );
    ...
    const bestLimit = findBestLimit(totalMinutesAttended, currentClassType.limits);
 ...
 */
function findBestJustification(message: string, justifications: SettingJustification[]): SettingJustification {
 ...
 */
function getDefaultLimit(minutesAttended: number): ClassTypesLimit {
 ...
 */
function findBestLimit(minutesAttended: number, limits: ClassTypesLimit[]): ClassTypesLimit | undefined {
...
export function calculateClassPresence(meeting: ZoomMeeting | ZoomMeetingPastInstance, users: Profile[]): number {
