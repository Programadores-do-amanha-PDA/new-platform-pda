 ...
 */
function filterValidUsers(users: Profile[]): Profile[] {
    return users.filter((user): user is Profile => !!user.email && typeof user.email === "string");
}
...
export function calculateUserWeeklyAttendance({
    ...
    availableJustifications,
    shouldAggregateInMetric = true,
}: CalculateUserWeeklyAttendancePropsT): CalculateUserAttendanceResult {
