...
interface ClassroomActions {
    ...
    createClassroomAsync: (classroomData: Partial<Classroom>) => Promise<boolean | string>;
    updateClassroomAsync: (classroomId: string, updates: Partial<Classroom>) => Promise<boolean>;
        ...
        (set, get) => ({
            ...
            createClassroomAsync: async (classroomData) => {
                try {
                    ...
                    if (
                        ...
                        toast.error({
                            title: "Erro!",
                            description: "Nome da turma é inválido ou já existe!",
                        });
                    ...
                    if (!classroomData.period) {
                        toast.error({
                            title: "Erro!",
                            description: "Período da turma é inválido!",
                        });
                    ...
                    toast.success({
                        title: "Sucesso!",
                        description: `Turma ${newClassroom.name} criada com sucesso!`,
                    });
                    return newClassroom.id;
                } catch (error) {
                    toast.error({
                        title: "Erro!",
                        description: "Erro ao criar nova turma!",
                    });
                    console.error(error);
                    return false;
                }
            ...
            deleteClassroomAsync: async (classroomId) => {
                try {
                    ...
                    set({
                        classrooms: get().classrooms.filter((classroom) => classroom.id !== classroomId),
                    });
                    toast.success({
                        title: "Sucesso!",
                    ...
                    toast.error({
                        title: "Erro!",
                        description: "Erro ao deletar turma. Tente novamente mais tarde!",
                    });
                    return false;
                }
            },
