...
import { CoodeshAssessment } from "@/features/classroom-coodesh/types";
import { filterVisibilityClassroomStudents } from "@/features/classrooms/utils";
import { ClassroomOverviewData, StudentOverview } from "@/features/classrooms/overview/types";
import { calculatePresenceByType, calculateCoodeshScores, calculateProjectNotes, calculateGeneralPresence } from "../utils";
import { filterDataByDateRange } from "@/features/shared/utils/filter-data-by-date-range";
import { Enrollment } from "@/features/enrollments";
import { ClassroomSetting, UserMode } from "@/features/classrooms/settings";
 ...
 * });
 */
export const useClassroomOverviewData = ({
    classroomId,
    users,
    currentSetting,
    ...
    dateRange,
    enrollmentsByUserId,
}: UseClassroomOverviewDataParams): ClassroomOverviewData => {
    return useMemo(() => {
        // Filter classroom students
        const classroomStudents: Profile[] = filterVisibilityClassroomStudents({
            users,
            classroomId,
        ...
        const studentsData: StudentOverview[] = classroomStudents.map((user, index) => {
            ...
            const userEnrollments = enrollmentsByUserId[user.id || ""]?.find(
