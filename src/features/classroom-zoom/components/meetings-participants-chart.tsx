...
export function MeetingsParticipantsChart({
    ...
    const [timeRange, setTimeRange] = useState<"all" | "90d" | "30d" | "7d">("30d");
    const [selectedAccountId, setSelectedAccountId] = useState<string>("all");

    /**
     * Memoized account options derived from chart data.
     * Computed once per unique set of accounts to avoid unnecessary re-renders.
     */
    const accountOptions = useMemo(() => {
        return Array.from(
            new Map(chartData.map((item) => [item.account_id, { id: item.account_id, label: item.account_label }])).values(),
        );
    }, [chartData]);

    const filteredData = chartData
        .filter((item) => selectedAccountId === "all" || item.account_id === selectedAccountId)
        .filter((item) => {
    ...
    const maxParticipants = Math.max(...filteredData.map((item) => item.participants), 0);
    const maxPollResults = Math.max(...filteredData.map((item) => item.poll_results), 0);
        ...
        <Card>
            <CardHeader className="w-full flex items-center justify-between gap-2 space-y-0 border-b sm:flex-row">
                <div className="flex flex-col gap-1 text-center sm:text-left">
                    ...
                    <CardDescription>Dados quantitativos de participação e respostas por reunião.</CardDescription>
                </div>
            ...
            <CardContent className="px-2 pt-4 sm:px-6 sm:pt-6">
                <ChartContainer config={chartConfig} className="aspect-auto h-[350px] w-full">
                    <AreaChart data={filteredData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                        ...
                        />
                        <defs>
                            <linearGradient id="fill-participants" x1="0" y1="0" x2="0" y2="1">
                                <stop offset="5%" stopColor="var(--chart-1)" stopOpacity={0.8} />
                                <stop offset="95%" stopColor="var(--chart-1)" stopOpacity={0.1} />
                            </linearGradient>
                            <linearGradient id="fill-poll_results" x1="0" y1="0" x2="0" y2="1">
