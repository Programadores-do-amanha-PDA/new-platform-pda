...
import { useZoomAPIStore } from "../../stores/api";
...
const MeetingsSheetData = ({ classroomId }: { classroomId: string }) => {
    ...
    const [classroomZoomAccounts, setClassroomZoomAccounts] = useState<ZoomAccountT[]>([]);
    ...
    useEffect(() => {
        const loadMeetings = async () => {
            if (openModal) {
                ...
                const filteredAccounts = accounts.filter((account) => account.classroom_id === classroomId);
    ...
    const classroomMeetingsByAPI = meetingsByAPI.filter((meeting) => accountIds.includes(meeting.account_id));

    const filteredMeetings = classroomMeetingsByAPI.filter((meeting) => {
        if (existingMeetingIds.has(meeting.meeting_id)) return false;
        if (meetingsSearch) {
            const searchMatch = meeting.topic.toLowerCase().includes(meetingsSearch.toLowerCase());
        ...
        <Sheet onOpenChange={setOpenModal} open={openModal}>
            ...
            <SheetContent className="flex flex-col h-full">
                <SheetHeader>
                    ...
                    <SheetDescription>Adicione abaixo as reuniões do Zoom que fazem parte desta turma</SheetDescription>
                </SheetHeader>
                    ...
                    <header className="flex flex-col items-end gap-4 p-2">
                        <InputGroup>
                            ...
                            {meetingsSearch.length > 0 && (
                                <InputGroupAddon align="inline-end">{filteredMeetings.length} resultados</InputGroupAddon>
                            )}
                        </InputGroup>
                    ...
                    {filteredMeetings.length > 0 ? (
                        ...
                        <div className="flex justify-center items-center w-full h-full">
                            <EmptyState
                                ...
                                title={meetingsSearch ? "Nenhuma reunião encontrada..." : "Não há reuniões disponíveis"}
                                description={
                                    meetingsSearch
                                ...
                                action={
                                    <SheetClose asChild>
                                        <Button variant="link" asChild>
                                            <Link href={`/dashboard/classrooms/${classroomId}/zoom/accounts`}>
