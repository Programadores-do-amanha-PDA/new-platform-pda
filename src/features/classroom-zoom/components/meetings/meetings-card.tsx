...
} from "../../types/meetings-card";
import { getMeetingPastInstances, getUpcomingOccurrences } from "../../utils/meeting-card-utils";
import {
    getMeetingType,
    isFutureMeeting,
    formatDateTime,
...
const MeetingHeader = ({ meeting, classroomId }: MeetingHeaderProps) => (
    <div className="flex flex-col gap-1">
        ...
        <p className="text-sm font-semibold text-muted-foreground truncate">{getMeetingType(meeting.type)}</p>
    </div>
);
...
const MeetingStats = ({ meeting, pastInstancesCount, upcomingOccurrencesCount }: MeetingStatsProps) => (
...
const RefreshButton = ({ loading, allMeetingLoading, onRefresh }: RefreshButtonProps) => (
    <Button
        size="icon"
...
export default function ZoomMeetingsCard({
    ...
    const meetingPastInstances = getMeetingPastInstances(pastInstances, meeting.id);
    ...
    const isRecurringMeeting = (RECURRING_MEETING_TYPES as readonly number[]).includes(meeting.type);
