...
import { ZoomMeetingParticipant } from "../types/meetings";
        ...
        const pastInstanciesData =
            pastInstances
                ...
                .flatMap((instance) => {
                    ...
                    instance?.participants?.forEach((participant: ZoomMeetingParticipant) => {
        ...
        const pastMeetingsData =
            meetings
                ...
                .flatMap((meeting) => {
                    ...
                    const account_label = account?.me?.display_name || "Conta sem nome";
                    const participantGroups = new Map<string, ZoomMeetingParticipant>();
                    meeting?.participants?.forEach((participant: ZoomMeetingParticipant) => {
                        const existing = participantGroups.get(participant.user_email);
                        if (!existing) {
                            participantGroups.set(participant.user_email, participant);
                        }
            ...
            <div className="w-full flex flex-col gap-4 p-6 rounded-lg bg-primary/10 pb-4">
                ...
                <ul className="w-full h-max flex items-start gap-4 overflow-x-auto pb-5">
                    {accounts.map((account) => (
                        <ZoomAccountCard account={account} key={account.id} expansive={false} />
                    ))}
                </ul>
            </div>

            <MeetingsParticipantsChart chartData={chartData()} classroomId={classroom_id} />
        </div>
    );
}
