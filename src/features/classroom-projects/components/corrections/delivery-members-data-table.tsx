...
export function DeliveryMembersDataTable({
            ...
            <div className="w-full h-full flex justify-center items-center p-2 border-r border-b">
                <Checkbox
                    checked={selectedMembers.some(
                        (d) => d.email === row.original.email && d.deliveryId === row.original.deliveryId,
                    )}
    ...
    const userColumn: ColumnDef<DeliveryMemberT> = {
        ...
        cell: ({ row }) => (
            <div className="w-full h-full flex items-center gap-3 p-2 border-r border-b">
                <Avatar className="h-8 w-8">
                    ...
                    <AvatarFallback>{getFirstLastInitials(row.original.name) || "U"}</AvatarFallback>
                </Avatar>
                <div className="flex flex-col">
                    <p className="text-sm font-bold capitalize">{row.original.name}</p>
                    <p className="text-sm text-muted-foreground lowercase">{row.original.email}</p>
                </div>
            </div>
        ),
        sortingFn: (rowA, rowB) => {
        ...
        filterFn: (row, _id, filterValue) => {
            ...
            return (
                row.original.name.toLowerCase().includes(searchTerm) || row.original.email.toLowerCase().includes(searchTerm)
            );
        },
    };
        ...
        cell: ({ row }) => {
            ...
            return (
                <div className="w-full h-full flex justify-center items-center px-2 border-b">
                    <p className={cn("font-semibold text-sm", Number(finalNote) >= 7 ? "text-green-600" : "text-destructive")}>
                        {finalNote || "N/A"}
                    </p>
                </div>
            );
    ...
    const statusColumn: ColumnDef<DeliveryMemberT> = {
        ...
        cell: ({ row }) => {
            ...
            const currentStatus = status !== "pending" ? status : isSent ? "sent" : "pending";

            return (
                <div className="w-full h-full flex justify-center items-center px-2 border-b border-l">
                    {currentStatus === "sent" && <span className="text-xs text-green-600 font-medium">✓ Enviado</span>}
                    {currentStatus === "sending" && <span className="text-xs text-blue-600 font-medium">📤 Enviando...</span>}
                    {currentStatus === "error" && <span className="text-xs text-red-600 font-medium">❌ Erro</span>}
                    {currentStatus === "pending" && <span className="text-xs text-muted-foreground">⏳ Pendente</span>}
                </div>
            );
        },
        enableSorting: false,
    ...
    const baseColumns = disableSelection ? [userColumn, noteColumn] : [selectColumn, userColumn, noteColumn];
