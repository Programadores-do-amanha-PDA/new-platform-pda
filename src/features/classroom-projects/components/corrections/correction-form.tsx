...
import { correctionFormSchema } from "../../utils/corrections";
...
const CorrectionForm = ({ classroomId, selectedDelivery, handleClose, project }: CorrectionFormPropsT) => {
    ...
    const { corrections, createCorrection, updateCorrection } = useClassroomProjectCorrectionsStore();
    ...
    const classroomUsers = users.filter((user) => enrollments.map((enrollment) => enrollment.user_id).includes(user.id));
    const currentCorrection = classroomCorrections?.find((correction) => correction.delivery_id === selectedDelivery.id);
    ...
    const projectRuleID = PROJECTS_RULES[project.rule_id] || ({} as Record<string, Record<string, string>>);
    const rulesLabels = Object.keys(projectRuleID);

    const handleSetRulesSelected = (ruleL: string, rule: string, ruleNote: number) => {
    ...
    const finalProjectPlaceholder =
        rulesSelected.map((r) => r.ruleNote)?.reduce((accum, curr) => accum + curr, 0) / rulesLabels?.length || 0;
    ...
    const getChangedFields = (
        ...
        if (JSON.stringify(current.rules_selected) !== JSON.stringify(newData.rules_selected)) {
            changes.rules_selected = newData.rules_selected;
        }

        if (JSON.stringify(current.hits_itens) !== JSON.stringify(newData.hits_itens)) {
            changes.hits_itens = newData.hits_itens;
        }

        if (JSON.stringify(current.improvements_itens) !== JSON.stringify(newData.improvements_itens)) {
            changes.improvements_itens = newData.improvements_itens;
        }

        if (JSON.stringify(current.next_itens) !== JSON.stringify(newData.next_itens)) {
    ...
    const onSubmit = async (data: CorrectionFormT) => {
        ...
        const formattedData = {
            ...
            improvements_itens: [data.improvements.item1, data.improvements.item2, data.improvements.item3].filter(
                (item): item is string => item !== undefined && item.trim() !== "",
            ),
        ...
        if (!currentCorrection?.id) {
            ...
            try {
                setLoading(true);
                const changedFields = getChangedFields(currentCorrection, formattedData);
    ...
    return (
        <section className="flex flex-col p-0! border-t w-full h-max">
            <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col space-y-6 overflow-hidden">
                    <section className="flex flex-col gap-4 p-4 w-full h-max">
                        <Label className="w-full font-semibold text-lg">Dados da Entrega</Label>
                        <section className="flex border border-gray-900/15 dark:border-gray-50/15 rounded-xl w-full h-max max-h-96 overflow-hidden">
                            <div className="flex flex-col p-0 border-gray-900/15 dark:border-gray-50/15 border-r w-max h-full">
                                <div className="flex justify-start items-center bg-muted px-4 border-gray-900/15 dark:border-gray-50 border-b w-full h-11">
                                    <p className="font-bold text-base">
                                        {project.project_type === "mini_project" ? "Autor" : "Integrantes"}
                                    </p>
                                </div>
                            ...
                            <div className="flex flex-col gap-4 p-4 w-full h-full">
                                {selectedDelivery.links && selectedDelivery.links.length > 0 && (
                                ...
                                {selectedDelivery.observation && selectedDelivery.observation.length > 0 && (
                                    <div className="flex flex-col gap-4 w-full h-max">
                                        <p className="font-bold text-base">Observação</p>
                                        <span className="px-4 font-normal text-sm">{selectedDelivery?.observation}</span>
                                    </div>
                                )}
                            </div>
                        </section>
                                ...
                                <FormField
                                    ...
                                    render={({ field }) => (
                                        <FormItem>
                                            ...
                                            <FormControl>
                                                <Input {...field} placeholder="Primeiro ponto forte" className="max-w-2xl" />
                                            </FormControl>
                                            <FormMessage />
                                        </FormItem>
                                    )}
                                ...
                                <FormField
                                    ...
                                    render={({ field }) => (
                                        <FormItem>
                                            ...
                                            <FormControl>
                                                <Input {...field} placeholder="Segundo ponto forte" className="max-w-2xl" />
                                            </FormControl>
                                            <FormMessage />
                                        </FormItem>
                                    )}
                                ...
                                <FormField
                                    ...
                                    render={({ field }) => (
                                        <FormItem>
                                            ...
                                            <FormControl>
                                                <Input {...field} placeholder="Terceiro ponto forte" className="max-w-2xl" />
                                            </FormControl>
                                            <FormMessage />
                                        </FormItem>
                                    )}
                            ...
                            <div className="flex flex-row flex-wrap gap-6 md:gap-0 p-4 py-6 w-full h-full">
                                <div className="w-full md:w-1/2 h-max md:h-full">
                                    <FormField
                                        ...
                                        render={({ field }) => (
                                            <FormItem>
                                                <FormLabel className="font-semibold text-base">*Nota final</FormLabel>
                                                <FormControl>
                                                    <Input
                                                        {...field}
                                                        type="number"
                                ...
                                <div className="w-full md:w-1/2 h-full">
                                    <FormField
                                        ...
                                        render={({ field }) => (
                                            <FormItem>
                                                <FormLabel className="font-semibold text-base">*Considerações Finais</FormLabel>
                                                <FormControl>
                                                    <Textarea
                                                        {...field}
                                                        rows={8}
                    ...
                    <section className="flex flex-col gap-4 px-4 w-full h-max">
                        <Accordion type="single" collapsible defaultValue="automatic-messages" className="w-full">
                                            ...
                                            <Item key={i} variant="outline" className="min-w-xs h-max">
                                                <ItemContent>
                                                    ...
                                                    <ItemDescription className="text-sm line-clamp-none">
                                                        {PROJECTS_RULES_FEEDBACKS[project.rule_id]?.[item.ruleL]?.[item.rule] ||
                    ...
                    <footer className="flex justify-between p-4 pt-8 border-gray-900/5 dark:border-gray-50/5 border-t-2 w-full h-max">
                        ...
                        <Button type="submit" variant="default" className="cursor-pointer" disabled={loading}>
