...
const RenderSquadMembers = ({ squadMembers, classroomUsers }: { squadMembers: string[]; classroomUsers: Profile[] }) => {
    const isSquadMembersJustEmails = squadMembers.every(
        (member) => REGEX_FOR_EMAIL_VALIDATION.test(member) || classroomUsers.some((u) => u?.email === member),
    );
            ...
            {squadMembers.map((memberId) => {
                ...
                return (
                    <Avatar key={memberId} className="border-3 border-background size-10">
                        ...
                        <AvatarGroupTooltip>
                            <p>{member?.full_name || memberId || "Participante Desconhecido"}</p>
                        </AvatarGroupTooltip>
                        <AvatarFallback>{getFirstLastInitials(member?.full_name || "") || memberId}</AvatarFallback>
                    </Avatar>
                );
            })}
        </AvatarGroup>
