...
import { ProjectRuleSelectorPropsT } from "../../types/projects/project-rule-selector";
import { projectsRules } from "../../utils/projects";

const ProjectRuleSelector = ({ handleSetRuleId, currentProjectRulesId }: ProjectRuleSelectorPropsT) => {
    const ruleIds = Object.keys(projectsRules);

    if (ruleIds.length === 0) {
        return (
            <div className="p-6 text-center border rounded-lg bg-muted/20">
                <p className="text-sm text-muted-foreground">Nenhuma rubrica disponível</p>
            </div>
        );
    }
            ...
            {ruleIds.map((ruleId) => {
                ...
                return (
                    ...
                    >
                        ...
                        <ItemContent>
                            <ItemTitle className={cn(isSelected ? "text-primary-foreground font-semibold" : "text-foreground")}>
                                {ruleId}
                            </ItemTitle>
