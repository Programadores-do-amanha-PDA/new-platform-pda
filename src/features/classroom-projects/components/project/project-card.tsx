...
import { ProjectCardPropsT } from "../../types/projects/project-card";
import { formatDateRangePeriod } from "../../utils/date";
import { isProjectActive, projectTypesLabels, generateProjectHref } from "../../utils/projects";
 ...
 */
const ProjectCard = ({ project, expansive, classroomId }: ProjectCardPropsT): JSX.Element => {
    const path = usePathname();
    const { deliveries } = useClassroomProjectDeliveriesStore();
    const { corrections } = useClassroomProjectCorrectionsStore();
    const [currentDelivery, setCurrentDelivery] = useState<ClassroomProjectDelivery | null>(null);
    ...
    const handleOpenCorrectionModal = (delivery: ClassroomProjectDelivery | null) => {
    ...
    return (
        <Item variant="default" role="article" aria-labelledby={`project-title-${project.id}`}>
            <ItemMedia
                role="img"
                title={projectTypesLabels[project.project_type].label}
                aria-label={`Tipo de projeto: ${projectTypesLabels[project.project_type].label}`}
            ...
            <ItemContent>
                <PermissionGuard
                    ...
                    fallback={
                        <ItemTitle className="text-base truncate cursor-default" id={`project-title-${project.id}`}>
                            {project.title}
                        </ItemTitle>
                    }
                >
            ...
            {isCurrentProjectActive && (
                <div className="flex flex-col items-start px-4 py-1.5 rounded-md">
                    ...
                    <p className="font-semibold text-primary-foreground">{formatDateRangePeriod(project.schedule_date)}</p>
                </div>
            )}
            <PermissionGuard permissions={["classroom_projects.update_all", "classroom_projects.update_self"]}>
            ...
            <RoleGuard roles={[RolesLabels.STUDENT]}>
