...
export default function ProjectDeliveriesCorrectionPage() {
    ...
    const [currentDelivery, setCurrentDelivery] = useState<ClassroomProjectDelivery | null>();
    const [selectedGroup, setSelectedGroup] = useState<GroupedDelivery | null>(null);
    const [isSendDeliveryFeedbackEmailModalOpen, setIsSendDeliveryFeedbackEmailModalOpen] = useState<boolean>(false);
    ...
    const enrollments = getEnrollmentsByClassroom(classroom_id);
    ...
    const allProjectCorrections = classroomCorrections?.filter((correction) => correction.project_id === project_id);
    ...
    const groupedDeliveries = useMemo(() => {
        ...
        if (currentProject.project_type === "mini_project") {
            grouped = groupDeliveriesByIndividual(allProjectDeliveries, classroomUsers);
        } else if (
    ...
    const firstUncorrectedDelivery = displayedDeliveries?.find((delivery) => {
        const hasDeliveryCorrection = allProjectCorrections.some((correction) => correction.delivery_id === delivery.id);
        return !hasDeliveryCorrection;
    });

    const emptyStateConfig = getEmptyStateConfig(allProjectDeliveries!, allProjectCorrections!, currentProject!);
    ...
    const handleSelectDelivery = (delivery: ClassroomProjectDelivery) => {
        if (currentDelivery?.id === delivery.id) {
            ...
            const deliveryGroup = groupedDeliveries.find((group) => group.deliveries.some((d) => d.id === delivery.id));
            ...
            <header className="flex flex-col gap-4 w-full h-max">
                <section className="flex flex-row justify-between">
                    <h3 className="font-semibold text-xl">
                        {currentProject.project_type === "mini_project" ? "Usuários" : "Squads"}
                    </h3>
                ...
                <section className="flex flex-col w-full">
                    {/* Lista de Grupos */}
                    <ul ref={groupListRef} className="flex gap-2 pb-4 w-full h-max overflow-x-auto">
                                            ...
                                            <div className="flex items-center gap-2">
                                                {currentProject.project_type === "mini_project" ? (
                                                    ...
                                                    <>
                                                        ...
                                                        <span className="font-medium text-sm">Squad {groupIndex + 1}</span>
                                                    </>
                                                )}
                                            </div>
                    ...
                    {selectedGroup && (
                        <div className="pt-2 border-t">
                            <div className="flex justify-between items-center mb-3">
                                ...
                                <Button variant="ghost" size="icon" onClick={handleUnselectGroup}>
                                    <X className="size-4" />
                                </Button>
                            </div>
                                ...
                                <ButtonGroup>
                                    {firstUncorrectedDelivery && (
                                        <Button
                                            ...
                                            onClick={() => handleSelectDelivery(firstUncorrectedDelivery)}
                                        >
                                            <Wand />
