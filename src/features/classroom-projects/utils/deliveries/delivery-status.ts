...
import { DeliveryStatusResultT } from "../../types/deliveries/delivery-status";
import { ClassroomProject } from "../../types/projects/project";
import { getProjectStatus } from "../projects";


/**
 * Determines if a recovery submission is available for a given project.
 *
 * Recovery is available when all of the following conditions are met:
 * 1. The user has a correction with a final note
 * 2. The final note is less than or equal to the project's cut-off grade
 * 3. The current date is within the project's recovery schedule period
 *
 * @param project - The classroom project containing recovery schedule and cut-off grade information
 * @param correction - Optional correction object containing the student's final note
 ...
 *   }
 * };
 * const correction = { final_note: 6.5 };
 *
 * const canRecover = isRecoveryAvailable(project, correction);
 * // Returns true if current date is within January 2024
 * ```
 */
export const isRecoveryAvailable = (project: ClassroomProject, correction?: ClassroomProjectCorrection): boolean => {
    ...
    const startDate = recoveryStart instanceof Date ? recoveryStart : new Date(recoveryStart);
    const endDate = recoveryEnd instanceof Date ? recoveryEnd : new Date(recoveryEnd);
 ...
 */
export const isDeliveryDuringRecovery = (project: ClassroomProject, delivery: ClassroomProjectDelivery): boolean => {
...
export const analyzeDeliveryStatus = (
    ...
    const projectDeliveries = deliveries?.filter((delivery) => delivery.project_id === project.id);
    ...
    const originalDelivery = userDeliveries.find((delivery) => !isDeliveryDuringRecovery(project, delivery));
    const recoveryDelivery = userDeliveries.find((delivery) => isDeliveryDuringRecovery(project, delivery));
    ...
    const originalCorrection = originalDelivery
        ? corrections?.find((correction) => correction.delivery_id === originalDelivery.id)
        : undefined;

    const recoveryCorrection = recoveryDelivery
        ? corrections?.find((correction) => correction.delivery_id === recoveryDelivery.id)
        : undefined;
    ...
    const latestCorrection = corrections?.find((correction) => correction.delivery_id === latestDelivery.id);
...
export const getProjectStatistics = (
    ...
    const projectCorrections = corrections?.filter((correction) => correction.project_id === project.id);
