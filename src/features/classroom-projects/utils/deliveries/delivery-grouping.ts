...
import { ClassroomProjectDelivery, GroupDeliveriesOptions, GroupedDelivery } from "../../types/deliveries/delivery";
...
export function groupDeliveriesBySquad(
    ...
    const { includeEmailMembers = true, memberSortFunction = defaultMemberSort } = options;
    ...
    deliveries.forEach((delivery) => {
        ...
        createOrUpdateGroup(groupedDeliveries, uniqueGroupKey, delivery, squadMembers, classroomUsers);
    });
 ...
 * ```
 */
function extractSquadMembers(delivery: ClassroomProjectDelivery, includeEmailMembers: boolean): Set<string> {
    ...
    if (includeEmailMembers && delivery.members && Array.isArray(delivery.members)) {
 ...
 * ```
 */
function generateGroupKey(squadMembers: Set<string>, sortFunction: (a: string, b: string) => number): string {
    const sortedMembers = Array.from(squadMembers).filter(Boolean).sort(sortFunction);
 ...
 */
function findPrimaryUser(userId: string, classroomUsers: Profile[]): Profile | undefined {
...
export function groupDeliveriesWithEmailMatching(
    ...
    const enhancedDeliveries = deliveries.map((delivery) => enhanceDeliveryWithUserIds(delivery, emailToUserIdMap));
 ...
 */
function createEmailToUserIdMap(classroomUsers: Profile[]): Map<string, string> {
