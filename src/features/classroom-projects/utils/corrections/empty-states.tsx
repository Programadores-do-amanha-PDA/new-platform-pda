import { CheckCircle2, Clock, Folders, ListChecks, ListTodo } from "lucide-react";
...
export const getEmptyStateConfig = (
    ...
    const allCorrected = hasDeliveries && allProjectCorrections?.length === allProjectDeliveries?.length;
    const isProjectStillActive = currentProject ? isProjectActive(currentProject) : false;
    const isProjectInFuture = currentProject ? isProjectFuture(currentProject) : false;
    ...
    if (hasDeliveries && !hasCorrections) {
        return {
            ...
            description: "Há entregas disponíveis para correção. Selecione uma entrega para começar",
        };
    }
    ...
    if (allCorrected) {
        return {
            ...
            description: "Parabéns! Todas as entregas foram corrigidas. Você pode enviar as correções por email",
        };
    }
