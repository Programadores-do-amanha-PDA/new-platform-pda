...
interface DeliveryState {
    deliveries: Record<string, ClassroomProjectDelivery[]>;
    loading: boolean;
}

interface DeliveryActions {
    setDeliveries: (classroomId: string, deliveries: ClassroomProjectDelivery[]) => void;
    getDeliveriesForClassroom: (classroomId: string) => ClassroomProjectDelivery[];
    ...
    createDelivery: (
        deliveryData: Omit<Partial<ClassroomProjectDelivery>, "id" | "created_at">,
        classroomId: string,
    ) => Promise<boolean>;
    updateDelivery: (id: string, deliveryData: Partial<ClassroomProjectDelivery>, classroomId: string) => Promise<boolean>;
...
export const useClassroomProjectDeliveriesStore = create<DeliveryState & DeliveryActions>()(
    devtools(
        (set, get) => ({
            ...initialState,
            ...
            getAllDeliveriesByProjectId: async (projectId) => {
                ...
                try {
                    ...
                    const deliveriesByClassroom = allDeliveries.reduce(
                        ...
                        {} as Record<string, ClassroomProjectDelivery[]>,
                    );
                    ...
                    set((state) => ({
                        deliveries: Object.keys(deliveriesByClassroom).reduce(
                            (acc, classroomId) => {
                                ...
                                acc[classroomId] = [...existing, ...deliveriesByClassroom[classroomId]];
                                return acc;
                            },
                            { ...state.deliveries } as Record<string, ClassroomProjectDelivery[]>,
                        ),
                    }));
                    return true;
                } catch (error) {
            ...
            getAllDeliveriesByClassroomId: async (classroomId) => {
                ...
                try {
                    ...
                    const allDeliveries = await getAllDeliveriesByClassroomId(classroomId);
                    if (!allDeliveries) throw new Error("No deliveries found");

                    const currentDeliveries = get().deliveries[classroomId] || [];
                    const existingIds = new Set(currentDeliveries.map((d) => d.id));
                    const newDeliveries = allDeliveries.filter((d) => !existingIds.has(d.id));
