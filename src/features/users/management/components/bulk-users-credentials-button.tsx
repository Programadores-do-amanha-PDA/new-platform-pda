...
export default function BulkUsersCredentialsButton({ selectedUsers, onComplete }: BulkUsersCredentialsButtonProps) {
        ...
        for (let i = 0; i < usersWithStatus.length; i++) {
            ...
            try {
                ...
                if (shortIds.length === 0) {
                    setUsers((prev) => prev.map((u, index) => (index === i ? { ...u, status: "skipped" } : u)));
                    skippedCount++;
                    continue;
                }
                ...
                if (result.status === 200 && result.data.status) {
                    setUsers((prev) => prev.map((u, index) => (index === i ? { ...u, status: "success" } : u)));
                    successCount++;
                } else {
                ...
                setUsers((prev) => prev.map((u, index) => (index === i ? { ...u, status: "error" } : u)));
                errorCount++;
            }
        }
            ...
            <DialogContent className="w-max max-w-[85vw] overflow-hidden">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        {stage === 0 && (
                            <>
                                Você está prestes a enviar emails com credenciais para <strong>{validEmails}</strong> usuário(s)
                                selecionado(s).
                        ...
                        <Table className="w-full h-full">
                            <TableHeader className="top-0 z-10 sticky bg-background shadow-sm">
                                <TableRow>
                                    ...
                                    <TableHead className="font-semibold">Turmas (Short IDs)</TableHead>
                                </TableRow>
                            </TableHeader>
                            <TableBody>
                                {users.map((user) => (
                                        ...
                                        <TableCell>
                                            <div className="flex items-center gap-3">
                                                <Avatar className="w-8 h-8">
                                                    ...
                                                    <AvatarFallback>{getFirstLastInitials(user.full_name)}</AvatarFallback>
                                                </Avatar>
                ...
                <DialogFooter>
                    {stage === 0 ? (
                        <>
                            ...
                            <Button onClick={handleSend} disabled={isLoading || validEmails === 0} className="gap-2">
