...
import { UserModeFormDataT, ClassroomConfigUserModeT } from "../types";
import { DEFAULT_FEATURE_EXCLUSIONS, getAllFeaturesRules } from ".";
...
export const createUserModeFromFormData = (
  ...
  return {
    ...
    featuresRules: formData.featuresRules.map((rule) => {
      const { hiddenFeatures, excludedFromMetrics } =
        DEFAULT_FEATURE_EXCLUSIONS;

      return {
        id: rule.id,
        isVisible: hiddenFeatures.includes(rule.id)
          ? undefined
          : rule.isVisible,
        aggregateInMetric: excludedFromMetrics.includes(rule.id)
          ? undefined
          : rule.aggregateInMetric,
      };
    }),
