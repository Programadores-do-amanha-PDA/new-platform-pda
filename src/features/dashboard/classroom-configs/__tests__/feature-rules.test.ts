...
jest.mock("@/providers/admin/sidebar-config", () => ({
    ADMIN_CLASSROOM_PAGES_KEYS: ["overview", "coodesh", "zoom", "students", "assignments", "content", "analytics"],
}));
...
describe("Feature Rules Utilities", () => {
    ...
    describe("DEFAULT_FEATURE_EXCLUSIONS", () => {
        it("should have correct default hidden features", () => {
            expect(DEFAULT_FEATURE_EXCLUSIONS.hiddenFeatures).toEqual(["overview", "coodesh", "zoom"]);
        });

        it("should have correct default excluded from metrics", () => {
            expect(DEFAULT_FEATURE_EXCLUSIONS.excludedFromMetrics).toEqual(["overview", "coodesh", "zoom"]);
        });
    });
