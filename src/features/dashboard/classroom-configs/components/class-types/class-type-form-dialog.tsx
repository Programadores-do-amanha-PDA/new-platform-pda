...
import { ClassroomConfigClassTypesT, ClassroomConfigClassTypesFormDataT } from "../../types";
...
const ClassTypeFormDialog = ({ currentClassType, configId, onClose, onSuccess, trigger }: ClassTypeFormDialogProps) => {
    ...
    const onSubmit = async (data: ClassroomConfigClassTypesFormDataT) => {
        ...
        try {
            ...
            const currentConfig = Object.values(configsByClassroom).find((config) => config.id === configId);
            if (!currentConfig) {
    ...
    return (
        <Dialog open={open} onOpenChange={handleOpenChange}>
            ...
            <DialogContent className="max-w-2xl w-full max-h-[90vh] h-full flex flex-col overflow-hidden">
                ...
                <Form {...form}>
                    <form onSubmit={form.handleSubmit(onSubmit)} className="h-full w-full flex flex-col gap-6 overflow-hidden">
                        <FormField
                            control={form.control}
                            name="title"
                            render={({ field }) => (
                        ...
                        <div className="w-full h-full flex flex-col overflow-hidden bg-muted rounded-lg">
                            ...
                            <ul className="w-full h-max p-4 flex overflow-y-auto flex-col gap-4">
                                {fields.map((field, index) => (
                                    <li key={field.id} className="border rounded-lg space-y-4 bg-background">
