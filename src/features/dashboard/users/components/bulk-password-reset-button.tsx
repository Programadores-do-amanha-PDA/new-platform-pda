...
import { sendPasswordResetToMultipleUsers } from "@/app/actions";
...
export default function BulkPasswordResetButton({
  selectedUsers,
  onComplete,
}: BulkPasswordResetButtonProps) {
  ...
  const handleSendPasswordReset = async () => {
    ...
    try {
      ...
      if (failed === 0) {
        toast.success(
          `Email de redefinição enviado para ${successful} usuário(s) com sucesso!`
        );
      } else {
        toast.warning(
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Você está prestes a enviar emails de redefinição de senha para{" "}
