...
import { UserModeFeatureRuleT, ClassroomConfigUserModeT } from "@/types";
...
export const DEFAULT_FEATURE_EXCLUSIONS = {
  hiddenFeatures: ["overview", "coodesh", "zoom"] as const,
  excludedFromMetrics: ["overview", "coodesh", "zoom"] as const,
} as const;

// Type safety for feature keys
export type ClassroomFeatureKey = (typeof ADMIN_CLASSROOM_PAGES_KEYS)[number];
export type ExcludedFeature =
  (typeof DEFAULT_FEATURE_EXCLUSIONS.hiddenFeatures)[number];
...
export const getAllFeaturesRules = (
  ...
  const createDefaultFeatureRules = (): UserModeFeatureRuleT[] => {
    return ADMIN_CLASSROOM_PAGES_KEYS.map(
      (feature): UserModeFeatureRuleT => ({
        id: feature,
        isVisible: !hiddenFeatures.includes(feature as ExcludedFeature),
        aggregateInMetric: !excludedFromMetrics.includes(
          feature as ExcludedFeature
        ),
      })
    );
  };
  ...
  const mergeWithUserModeRules = (
    ...
    if (!currentUserMode || !currentUserMode.featuresRules || currentUserMode.featuresRules.length === 0) {
      return defaultRules;
    }
