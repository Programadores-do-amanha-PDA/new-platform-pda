...
import { getProfileById, AuthUserWithProfile, Profile } from "@/features/dashboard/profile";
        ...
        (set) => ({
            ...
            fetchAllUsersWithProfiles: async ({ role }: FetchAllUsersWithProfilesProps) => {
                try {
                    ...
                    if (!users) throw new Error("no get all users response");
            ...
            createNewUser: async ({ userData }: CreateNewUserProps) => {
                try {
                    ...
                    const userWithProfile: AuthUserWithProfile = {
                        ...
                        profile: {
                            ...userProfileResponse,
                            enrollments: userProfileResponse.enrollments || [],
                        },
                    };
                    ...
                    set((state) => ({
                        users: state.users.map((currentUser) => {
                            if (currentUser.id === id) {
                                const userUpdatedData: AuthUserWithProfile = {
                                    ...currentUser,
                                    ...userUpdatedResponse,
                                    profile: {
