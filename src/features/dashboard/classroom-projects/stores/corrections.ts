...
interface CorrectionState {
  corrections: Record<string, ClassroomProjectCorrectionT[]>;
  loading: boolean;
}

interface CorrectionActions {
  setCorrections: (classroomId: string, corrections: ClassroomProjectCorrectionT[]) => void;
  getCorrectionsForClassroom: (classroomId: string) => ClassroomProjectCorrectionT[];
    ...
    (set, get) => ({
      ...
      getAllCorrectionsByProjectId: async (projectId) => {
        ...
        try {
          ...
          const correctionsByClassroom = allCorrections.reduce((acc, correction) => {
            ...
            return acc;
          }, {} as Record<string, ClassroomProjectCorrectionT[]>);
          
          // Merge with existing corrections
          set((state) => ({
            corrections: Object.keys(correctionsByClassroom).reduce((acc, classroomId) => {
              const existing = state.corrections[classroomId] || [];
              acc[classroomId] = [...existing, ...correctionsByClassroom[classroomId]];
              return acc;
            }, { ...state.corrections } as Record<string, ClassroomProjectCorrectionT[]>),
          }));
          return true;
        } catch (error) {
          console.error(error);
      ...
      getAllCorrectionsByDeliveryId: async (deliveryId, classroomId) => {
        ...
        try {
          ...
          const currentCorrections = get().corrections[classroomId] || [];
          const existingIds = new Set(currentCorrections.map((c) => c.id));
          const newCorrections = allCorrections.filter(
      ...
      deleteCorrection: async (id, classroomId) => {
        try {
          ...
          set((state) => ({
            corrections: {
              ...state.corrections,
              [classroomId]: currentCorrections.filter(
