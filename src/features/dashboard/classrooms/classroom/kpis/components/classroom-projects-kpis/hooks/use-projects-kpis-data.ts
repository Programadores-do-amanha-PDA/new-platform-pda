...
export const useProjectsKPIsData = ({ classroomId }: UseProjectsKPIsDataParams): UseProjectsKPIsDataReturn => {
    // Use shallow selectors to prevent re-renders when unrelated store state changes
    ...
    const classroomUsers = useMemo(() => {
        ...
        return users.filter((user) => user.profile?.enrollments?.some((enrollment) => enrollment.classroom_id === classroomId));
    }, [users, classroomId]);

    const allAggregatedMetricUsers = useMemo(
        () => filterMetricClassroomStudents({ users: classroomUsers, classroomId, userModes, ruleId: "projects" }),
        // eslint-disable-next-line react-hooks/exhaustive-deps
