"use server";
import { ZoomMeetingPastInstance, ZoomMeeting } from "../types";
...
export const getAllMeetingsByAccount = async (ZOOM_ACCESS_TOKEN: string) => {
  try {
    ...
    return meetings.flatMap((meeting) => {
      const { id, ...restMeetingData }: ZoomMeeting = meeting;

      return {
        ...restMeetingData,
        meeting_id: Number(id),
      } as Omit<ZoomMeeting, "id">;
    }) as Omit<ZoomMeeting, "id">[];
  } catch (error) {
...
export const getMeetingById = async (
  ...
  try {
    ...
    const { id, created_at, ...restMeetingData }: ZoomMeeting = response.data;
...
export const getPastMeetingInstances = async (
  ...
  try {
    ...
    return (
      (response.data.meetings?.filter(Boolean) as ZoomMeetingPastInstance[]) ||
      []
    );
  } catch (error) {
