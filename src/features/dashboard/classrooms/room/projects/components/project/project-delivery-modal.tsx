...
import { ClassroomProject, ClassroomProjectDelivery } from "../../types";
...
const ProjectDeliveryModal = ({ project, classroomId, isOpen, onClose, currentDelivery }: ProjectDeliveryModalProps) => {
    ...
    const classroomUsers = users
        .filter((u) => u.profile?.enrollments?.some((c) => c.classroom_id === classroomId))
        .filter(Boolean);
    ...
    const deliveryAuthor = currentDelivery?.id ? classroomUsers.find((u) => u.id === currentDelivery.user_id) : user;
                            ...
                            <div className="space-y-4 mt-2">
                                ...
                                <div className="p-4 border border-primary rounded-xl">
                                    <div className="flex items-center gap-3">
                                        <Avatar>
                                            <AvatarImage src={deliveryAuthor?.profile.avatar_url || ""} />
                                            <AvatarFallback>
                                                {deliveryAuthor?.profile.full_name
                                                    ?.split(" ")
                                                    .map((n) => n[0])
                                                    .join("")
                                                    .toUpperCase()}
                                        ...
                                        <div>
                                            <p className="font-bold text-sm">{deliveryAuthor?.profile.full_name}</p>
