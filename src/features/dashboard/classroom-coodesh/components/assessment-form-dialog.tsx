...
const AssessmentFormDialog = ({
  ...
  const { createManualAssessment, updateAssessment } =
  ...
  useEffect(() => {
    form.reset({
      ...
      default_locale:
        (currentAssessment?.default_locale as "pt" | "en" | "es") ?? "pt",
      duration: currentAssessment?.duration ?? 90,
      duration_unit:
        (currentAssessment?.duration_unit as "minute" | "hour") ?? "minute",
    });
  ...
  const handleSubmit = async (data: AssessmentFormData) => {
    ...
    try {
      if (onSubmit) {
        ...
        throw new Error(
          "classroom_id is required for creating new assessments"
        );
      }
      ...
      console.error(error);
      toast.error(
        `Erro ao ${currentAssessment ? "editar" : "criar"} a avaliação!`
      );
    } finally {
      setLoading(false);
    }
