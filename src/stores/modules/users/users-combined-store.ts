...
interface UsersCombinedActions {
    ...
    createNewUser: (userData: Partial<AuthUser & { password: string }>) => Promise<string | false>;
    updateUser: (userID: string | undefined, updates: Partial<AuthUser & { password: string }>) => Promise<boolean>;
...
export const useUsersCombinedStore = create<UsersCombinedState & UsersCombinedActions>()(
    devtools(
        () => ({
            // Getters
            getUsers: () => useUsersStore.getState().users,
            isLoading: () => useUsersStore.getState().loading || useUserClassroomsStore.getState().loading,

            // User management actions
            getAllUsersWithProfiles: async (role?: RolesT) => {
                const success = await useUsersStore.getState().getAllUsersWithProfiles(role);
                if (success) {
