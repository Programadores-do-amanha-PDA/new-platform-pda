...
type ZoomAccountsStoreInitProps = {
...
// Create a store creator function that accepts the API function
export const createZoomAccountStore = ({
  getZoomMeAccountDataByAPI,
}: ZoomAccountsStoreInitProps) => {
  ...
  return create<ZoomAccountState & ZoomAccountActions>()(
    devtools(
      (set, get) => ({
        ...initialState,
        ...
        createAccount: async (accountData) => {
          try {
            ...
            const me = await get().getZoomMeAccountDataByAPI(
              accountData.account_id,
              accountData.client_id,
              accountData.client_secret
            );
        ...
        reset: () => {
          const { getZoomMeAccountDataByAPI } = get();
          set({ ...initialState, getZoomMeAccountDataByAPI });
        },
      }),
